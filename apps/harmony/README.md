...
This is the small HarmonyOS native app for the existing Rust bridge. The Gallery tab fills its page with a single-select `PhotoPickerComponent`, using HarmonyOS 7 API 26's CURRENT original-format preference and high-resolution HEIC capability; each selected photo is physically copied into the app sandbox before Rust reads it. Tapping a photo opens one oversized, rounded preview card with a glass information and conversion panel. The app also receives supported HEIC/JPEG shares into sandbox files, runs a bounded foreground conversion queue, exports completed results through one serial multi-file save session, and can inspect and split Motion Photos, create eligible Live Photo pairs, and export paired HEIC/MOV files. Settings and queue recovery stay in same-page panels so opening them does not destroy the queue session.
