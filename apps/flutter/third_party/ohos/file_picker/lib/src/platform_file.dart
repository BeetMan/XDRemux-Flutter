...
class PlatformFile {
  ...
  /// v12 compat: a Uri view of the file. Local path when available, else the
  /// platform identifier (content:// on Android).
  Uri get uri => _path != null
      ? Uri.file(_path!)
      : (identifier != null ? Uri.parse(identifier!) : Uri());

  /// v12 compat: read the file bytes on demand (falls back to the eagerly
  /// loaded bytes when present).
  Future<Uint8List> readAsBytes() async {
    if (bytes != null) return bytes!;
    if (_path != null) return File(_path!).readAsBytes();
    throw StateError('PlatformFile has neither bytes nor path');
  }
