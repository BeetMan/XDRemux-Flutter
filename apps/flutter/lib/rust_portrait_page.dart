...
class _RustPortraitPageState extends State<RustPortraitPage> {
  ...
  String _diagnosisSummary(Map<String, dynamic> report) {
    if (report['available'] != true) {
      return report['classification']?.toString() ?? '诊断不可用';
    }
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _outputDirectory = directory;
          _status = '完成：$passed/${_results.length} 张通过 Rust 人像结构验证。';
        });
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Rust 人像模式实验室')),
      body: ListView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  FilledButton.icon(
                    ...
                    label: Text(_running ? '处理中…' : '诊断并生成 Rust 人像输出'),
                  ),
                  const SizedBox(height: 10),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('诊断结果', style: theme.textTheme.titleMedium),
                        const Spacer(),
