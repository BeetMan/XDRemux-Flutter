...
- **人像**：底图取自尾部 `src.image`（Ultra HDR JPEG，自带 GainMap），主图与 tmap 同尺寸、GainMap 为其 0.5×；`src.image` 的 EXIF 旋转烘焙进主图像素（`irot`=0），最终 EXIF Orientation 归一为 1，拍摄 EXIF 从原图恢复（见 `modules/portrait-pipeline.md` §2.1）。深度/蒙版按 `src.image` 几何等比映射；标定走 `rear.depth.config` 的每张照片深度曲线（详见 `modules/portrait-pipeline.md` §2.1 / §3）
