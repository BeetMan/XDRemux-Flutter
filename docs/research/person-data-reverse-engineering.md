...
## 6.6 76 点模板（从 Apple 样张反推）

**方法**：把 26 个实例的 76 点归一化到 faceROI（平移到框中心、按框尺寸缩放），
取平均 → canonical 人脸模板。**模板直接来自 Apple 自己的数据**，无需外部模型。

**放置精度**（用样本自己的 faceROI 放置模板，测与真值偏差）：

| 指标 | 误差（相对 faceROI 尺寸 = 1.0）|
|---|---|
| 每点平均 | **0.103** |
| 中位 | 0.091 |
| 90 分位 | 0.171 |
...
**模板离散度**：mean std = 0.218（max 0.46）—— 主要来自**姿态差异**
（yaw/pitch/roll），纯位置归一化不补偿姿态。

**已知局限**：10% 的误差对柔肤（参考肤色 + 粗糙度）足够，但对**眼部 ROI**
（`UnderEyeBrightening`）偏粗 —— 眼点误差 0.107 ≈ 眼框宽度量级。

**改进方向**：
1. 用 YuNet 的 5 点（两眼/鼻/两嘴角）做**相似变换**锚定（比纯 bbox 更能
   处理头部旋转）
2. 用 `faceYaw/Pitch/Roll` 做姿态补偿后再平均（应显著降低离散度 0.218）
3. 精度不够时接关键点模型（PFLD 106 点 → 映射 76）

模板存 `xdremux/rust/models/face_landmarks_76.json`（76×2 归一化坐标 + std）。
