...
This is the HarmonyOS native app for the existing Rust bridge. Its Gallery tab is an app-owned, full-screen three-column media-library grid with paged thumbnails; tapping a photo copies its original bytes into the app sandbox, then shows the oversized detail card with format, dimensions, EXIF information, classification, and conversion controls. The app requests `ohos.permission.READ_IMAGEVIDEO` for this whole-library experience. Huawei currently restricts approval of that permission to media cloning, backup, or synchronization scenarios, so AppGallery publication depends on Huawei approving the app's actual photo-conversion use case. The queue's multi-file PhotoViewPicker alone requests API 26 CURRENT originals and high-resolution HEIC; it also physically copies every selected URI into the queue sandbox before Rust reads it. The app uses a transparent immersive status bar and HDS Navigation detail title bar with adaptive material. It also receives supported HEIC/JPEG shares into sandbox files, runs a bounded foreground conversion queue, exports completed results through one serial multi-file save session, and can inspect and split Motion Photos, create eligible Live Photo pairs, and export paired HEIC/MOV files. Settings and queue recovery stay in same-page panels so opening them does not destroy the queue session.
