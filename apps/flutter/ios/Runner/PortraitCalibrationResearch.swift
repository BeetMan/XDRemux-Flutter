...
/// Explicitly gated Portrait calibration runner (macOS + iOS research).
///
/// This is a research tool, not a conversion backend capability. It patches
/// only a scratch copy of `rear.depth`, calls the vendored upstream Swift
...
public enum PortraitCalibrationResearch {
    ...
    private static func runOne(
        ...
        do {
            diagnostic = try PortraitDepthDiagnostics.report(for: inputURL)
        } catch {
            throw ResearchError.invalidInput(
                "Portrait diagnostic failed: \(error)"
            )
        }
    ...
    private static func runZstd(
        ...
        // iOS: no subprocesses; the vendored embedded zstd 1.5.7 handles
        // both directions in-process. The executableURL argument is the
        // sentinel from PortraitDepthDiagnostics and is ignored.
        if decompress {
            return try EmbeddedZstdDecoder.decode(data)
        }
        return try EmbeddedZstdDecoder.encode(data)
    }
