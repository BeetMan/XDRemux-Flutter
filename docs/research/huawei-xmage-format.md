...
- 因此不能把 233644 缺少 `xtstyle` 解释为“没有选择鲜艳”。更合理的第一假设是：
  **高像素/标准合并像素、镜头或相机内部处理路径不同，决定了是否序列化 xtstyle**；风格可能已烘焙进图像，或标准路径使用了更小的参数表示。
- 需要受控实验：同一场景、同一镜头、同一「鲜艳」，分别切换标准/高像素（以及不同焦段），逐张比较 item 清单。
...
- [ ] 233642/233646（4320×5760）与 233644（3072×4096）做同场景标准/高像素受控对照，确认 xtstyle 的触发条件
